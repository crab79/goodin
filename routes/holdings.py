...
# 根據你的資料庫設定修改這行
engine = create_engine("mysql+mysqlconnector://root:password@localhost/goodin", echo=False)
    ...
    with engine.connect() as conn:
        result = conn.execute(text("""
            SELECT h.stock_id, s.symbol, s.name, h.holding_type,
                   h.available_quantity, h.total_quantity, h.avg_price,
...
def get_live_holdings():
    ...
    with engine.connect() as conn:
        result = conn.execute(text("""
            ...
            FROM holdings h
            JOIN stocks s ON h.stock_id = s.stock_id
            LEFT JOIN realtime_stock rs ON s.price_id = rs.price_id
